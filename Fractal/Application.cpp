...
{
	std::string v_code, p_code;
	{
		std::ifstream file("VertexShader.hlsl");
		if (!file)
			return false;
		std::stringstream buffer;
		buffer << file.rdbuf();
		v_code.assign(buffer.str());
	}
	{
		std::ifstream file("PixelShader.hlsl");
		if (!file)
			return false;
		std::stringstream buffer;
		buffer << file.rdbuf();
		p_code.assign(buffer.str());
	}
	...
	D3DCompile(v_code.c_str(), v_code.size(), "vshader", nullptr, nullptr, "vs_main", "vs_5_0", flags, 0, &v_compiled, &v_error);
	if (v_error)
	{
	...
	D3DCompile(p_code.c_str(), p_code.size(), "pshader", nullptr, nullptr, "ps_main", "ps_5_0", flags, 0, &p_compiled, &p_error);
	if (p_error)
	{
